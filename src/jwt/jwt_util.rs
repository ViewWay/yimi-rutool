...
impl JwtUtil {
    ...
    /// - Claims serialization fails
    /// - Token encoding fails
    pub fn create_token_with_algorithm(
        claims: &Claims,
        secret: &str,
        algorithm: Algorithm,
    ) -> JwtResult<String> {
        if !algorithm.is_hmac() {
            return Err(JwtError::invalid_algorithm(
                "Only HMAC algorithms supported with string secret",
            ));
        }
    ...
    #[cfg(feature = "jsonwebtoken")]
    fn create_token_with_jsonwebtoken(
        claims: &Claims,
        key: &SigningKey,
        algorithm: Algorithm,
    ) -> JwtResult<String> {
    ...
    /// - Token has expired
    /// - Token is not yet valid
    pub fn validate_token_with_algorithm(
        token: &str,
        secret: &str,
        algorithm: Algorithm,
    ) -> JwtResult<Claims> {
    ...
    fn validate_token_manual(
        ...
        if header.alg != algorithm.as_str() {
            return Err(JwtError::invalid_algorithm(format!(
                "Expected {}, got {}",
                algorithm.as_str(),
                header.alg
            )));
    ...
    fn sign(data: &str, key: &SigningKey, algorithm: Algorithm) -> JwtResult<Vec<u8>> {
        match (key, algorithm) {
            ...
            _ => Err(JwtError::invalid_algorithm(
                "Algorithm not supported in manual mode",
            )),
        }
    }

...
impl BitMap {
    ...
    pub fn count_ones(&self) -> usize {
        ...
        if let Some(&last_word) = self.data.last() {
            let bits_in_last_word = if self.size % 64 == 0 {
                64
            } else {
                self.size % 64
            };
    ...
    pub fn and(&mut self, other: &BitMap) {
        assert_eq!(
            ...
            "BitMap sizes must match for AND operation"
        );
    ...
    pub fn or(&mut self, other: &BitMap) {
        assert_eq!(
            self.size, other.size,
            "BitMap sizes must match for OR operation"
        );
    ...
    pub fn xor(&mut self, other: &BitMap) {
        assert_eq!(
            self.size, other.size,
            "BitMap sizes must match for XOR operation"
        );
